<template>
  <div class="content-section" :aria-busy="loading">
    <div v-if="loading" class="list-loading" aria-live="polite">
      <SkeletonUserList :count="4" action />
    </div>
    <div v-else class="content-body">
      <div class="user-list">
        <div v-if="items.length" class="users">
          ...
          >
            <img
              :src="item.targetUser?.avatar || defaultAvatar"
              class="user-avatar"
              alt="用户头像"
              @click="emit('open', item.targetUser?.id)"
            >
            <div class="user-info">
              <h4 class="user-name" @click="emit('open', item.targetUser?.id)">{{ item.targetUser?.nickName || '用户' }}</h4>
              <p class="user-intro">{{ item.targetUser?.introduction || '暂无简介' }}</p>
            </div>
            <SButton variant="glass" size="small" @click.stop="emit('unfollow', item.id)">取消关注</SButton>
          </SSurfacePanel>
        </div>
        <div v-else class="empty-tip">还没有关注任何人，去发现感兴趣的作者吧。</div>
...
import type { UserFollowItem } from '@/modules/user/composables/useUserCenter'

const props = defineProps<{ loading: boolean; items: UserFollowItem[]; page: number; size: number; total: number }>()
const emit = defineEmits<{
  open: [userId: number | undefined]
