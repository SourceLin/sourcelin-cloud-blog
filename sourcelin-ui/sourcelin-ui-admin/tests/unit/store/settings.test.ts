...
describe("useSettingsStore", () => {
  ...
  describe("重置设置", () => {
    it("应该重置所有设置为默认值", () => {
      ...
      expect(store.showTagsView).toBe(defaults.showTagsView);
      expect(store.showAppLogo).toBe(defaults.showAppLogo);
      expect(store.pageSwitchingAnimation).toBe(defaults.pageSwitchingAnimation);
      expect(store.layout).toBe(defaults.layout);
      expect(store.sidebarColorScheme).toBe(defaults.sidebarColorScheme);
      expect(store.theme).toBe(defaults.theme);
      expect(store.themeColor).toBe(defaults.themeColor);
    });
  });
});
