...
export const useUserStore = defineStore("user", () => {
  ...
  async function login(loginRequest: LoginRequest): Promise<void> {
    ...
    // 当前认证中心以 Sa-Token 单 token 机制运行，refreshToken 位保留兼容。
    AuthStorage.setTokens(token, token, rememberMe.value);
  }
  ...
  async function doRefreshToken(): Promise<void> {
    ...
    AuthStorage.setTokens(accessToken, accessToken, AuthStorage.getRememberMe());
  }
