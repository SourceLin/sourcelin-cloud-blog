...
export const hasPerm: Directive = {
  mounted(el: HTMLElement, binding: DirectiveBinding) {
    ...
    const requireAllPerm =
      (Array.isArray(requiredPerms) && requiredPerms.includes("*:*:*")) ||
      requiredPerms === "*:*:*";
    const hasAllPerm = permList.includes("*:*:*");

    // 超级管理员拥有所有权限，如果是"*:*:*"权限标识，则不需要进行权限校验
    if (roleList.includes(ROLE_ROOT) || hasAllPerm || requireAllPerm) {
      return;
    }

    // 检查权限
    const hasAuth = Array.isArray(requiredPerms)
      ? requiredPerms.some((perm) => permList.includes(perm))
      : permList.includes(requiredPerms);
...
export const hasRole: Directive = {
  mounted(el: HTMLElement, binding: DirectiveBinding) {
    ...
    const hasAuth = Array.isArray(requiredRoles)
      ? requiredRoles.some((role) => roleList.includes(role))
      : roleList.includes(requiredRoles);
