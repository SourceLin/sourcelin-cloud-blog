    ...
    <el-card shadow="hover" class="table-section">
      <div class="table-section__toolbar">
        <div class="table-section__toolbar--actions">
          <el-button
            v-hasPerm="['system:notice:create']"
            type="success"
            :icon="Plus"
            @click="openDialog()"
          >
      ...
      >
        ...
        <el-table-column align="center" label="通知类型" width="150">
          <template #default="scope">
            <DictTag v-model="scope.row.type" code="sys_notice_type" />
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="发布状态" min-width="100">
          <template #default="scope">
            <el-tag :type="getPublishStatusTagType(scope.row.publishStatus)">
        ...
        <el-table-column align="center" fixed="right" label="操作" width="150">
          <template #default="scope">
            ...
            <el-button
              v-if="scope.row.publishStatus != 1"
              v-hasPerm="['system:notice:update']"
              type="primary"
              size="small"
              link
    ...
    >
      ...
      <el-form ref="dataFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="通知类型" prop="type">
          <DictSelect v-model="formData.type" code="sys_notice_type" />
        </el-form-item>
        <el-form-item label="目标类型" prop="targetType">
          <el-select v-model="formData.targetType" style="width: 180px">
            <el-option
    ...
    >
      ...
      <el-descriptions :column="1">
        ...
        <el-descriptions-item label="发布状态：">
          <el-tag :type="getPublishStatusTagType(currentNotice.publishStatus)">
            {{ getPublishStatusLabel(currentNotice.publishStatus) }}
          </el-tag>
        </el-descriptions-item>
...
const publishStatusOptions = [
  { value: 0, label: "未发布", tagType: "info" },
  { value: 1, label: "已发布", tagType: "success" },
  { value: -1, label: "已撤回", tagType: "warning" },
] as const;

const targetTypeOptions = [
  { value: 1, label: "全体", tagType: "warning" },
  { value: 2, label: "指定", tagType: "success" },
] as const;

function getPublishStatusLabel(value?: number): string {
  return publishStatusOptions.find((item) => item.value === Number(value))?.label ?? "-";
}

function getPublishStatusTagType(value?: number): "success" | "warning" | "danger" | "info" {
  return (publishStatusOptions.find((item) => item.value === Number(value))?.tagType ?? "info") as
    | "success"
    | "warning"
    | "danger"
    | "info";
...
function getTargetTypeLabel(value?: number): string {
  return targetTypeOptions.find((item) => item.value === Number(value))?.label ?? "-";
}

function getTargetTypeTagType(value?: number): "success" | "warning" | "danger" | "info" {
  return (targetTypeOptions.find((item) => item.value === Number(value))?.tagType ?? "info") as
    | "success"
    | "warning"
    | "danger"
    | "info";
