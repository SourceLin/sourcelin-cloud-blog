            ...
            <div class="user-info">
              ...
              <el-descriptions :column="1" size="small" border class="profile-desc">
                <el-descriptions-item>
                  ...
                  <el-icon v-if="String(userProfile.sex) === '0'" class="gender-icon male">
                    <Male />
                  </el-icon>
                  <el-icon v-else-if="String(userProfile.sex) === '1'" class="gender-icon female">
                    <Female />
                  </el-icon>
                </el-descriptions-item>
                <el-descriptions-item>
    ...
    <el-dialog v-model="dialogState.visible" :title="dialogState.title" :width="500">
      ...
      >
        ...
        <el-form-item label="性别">
          <DictSelect v-model="userProfileForm.sex" code="sys_user_sex" />
        </el-form-item>
      </el-form>
...
const mobileSecurityDesc = computed(() => {
  return userProfile.value.phonenumber
    ? `已绑定：${maskMobile(userProfile.value.phonenumber)}`
    : "未绑定手机号，建议立即绑定";
});
...
const handleOpenDialog = (type: DialogType) => {
  ...
  switch (type) {
    ...
    case DialogType.MOBILE:
      dialogState.title = userProfile.value.phonenumber ? "更换手机号" : "绑定手机号";
...
async function handleUnbindMobile() {
  ...
  try {
    await ElMessageBox.prompt("请输入当前密码以解绑手机号", "解绑手机号", {
      type: "warning",
...
async function handleUnbindEmail() {
  ...
  try {
    await ElMessageBox.prompt("请输入当前密码以解绑邮箱", "解绑邮箱", {
      type: "warning",
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      inputType: "password",
...
const handleSubmit = async () => {
  try {
    if (dialogState.type === DialogType.ACCOUNT) {
      ...
      await UserAPI.bindOrChangeMobile();
      ElMessage.success(userProfile.value.phonenumber ? "手机号更换成功" : "手机号绑定成功");
      dialogState.visible = false;
      await loadUserProfile();
      ...
      await UserAPI.bindOrChangeEmail();
  ...
  .user-info {
    ...
    .avatar-wrapper {
      ...
      .avatar-edit-btn {
        ...
        right: 0;
        bottom: 0;
        background: color-mix(in srgb, var(--el-text-color-primary) 50%, transparent);
        border: none;
        transition: all 0.3s ease;

        &:hover {
          background: color-mix(in srgb, var(--el-text-color-primary) 70%, transparent);
        }
      }
    }
