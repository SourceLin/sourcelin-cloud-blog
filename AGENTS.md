...
- `scripts/**` 不进入开源分支提交。
- `docs/**` 可以进入开源分支，但 `docs/internal/**` 不提交。
...
3. 新增页面必须正确归入主包或分包：Tab 页入顶层 `pages`，业务页入对应 `subPackages`；主包仅 4 个页面（3 个原生 TabBar + 1 个液态 TabBar），体积 < 2MB。
...
7. `onPageScroll` 必须使用 `useBackToTop`（`src/shared/composables/useBackToTop`）的 `handlePageScroll`，禁止逐像素写响应式状态。
...
- MUST NOT：开源分支提交 `scripts/**`、`docs/internal/**`。
- MUST NOT：回退用户已有无关改动。
