...
| 导航 | 底部 TabBar + 页面内分段筛选 | P0 TabBar 建议：首页、发现、我的（3 个原生 TabBar）；圈子通过自定义液态 TabBar 承载，消息降级为“我的”页入口 |
...
移动端页面不要直接复刻 PC 顶部导航。PC 顶部导航包含首页、热门、文章、说说、树洞、关于等入口，移动端应改为三个原生底部 Tab（首页、发现、我的）+ 一个液态 Tab（圈子）。热门、分类、标签、导航、友链和关于全部通过发现页或我的页二级入口承载。这样能避免入口过多导致认知负担，也更符合优秀小程序“少入口、高转化、强路径”的设计标准。
...
| 社区与用户资产 | 圈子双列表（说说/树洞）、树洞投递、说说发布、我的互动、消息中心、资料编辑、体验设置已全部接入 | 社区氛围与用户回访入口已形成完整组合 | 后续以运营数据和低频专项优化为主 |
...
| 阶段 | 必须验收 | 不通过示例 |
|---|---|---|
| M1 基础整改 | 四个 Tab 可正常打开；已展示入口均可跳转；全局 token、请求封装、`ApiResponse`/`PageResult` 类型进入统一层；液态 TabBar 正常渲染 | 点击“登录/资料”报路由不存在；页面仍直接写普通蓝白卡片样式 |
...
- `sourcelin-ui/sourcelin-ui-uniapp/src/manifest.json` 中 `mp-weixin.appid` 已配置为真实 AppID（`wx65d3af978904a17c`），构建时通过 `build-tools/with-local-wechat-appid.mjs` 注入本地值以覆盖。
