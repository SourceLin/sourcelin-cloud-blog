...
| 模块 | 当前状态 | 产品判断 | 下一步 |
|---|---|---|---|
| 工程基线 | 已有 Uniapp、Vue 3、TypeScript、Pinia、Vite 脚手架，目录、TabBar、共享类型和请求层已落地 | 可作为现有主线继续演进，不需要重建项目 | 继续在现有工程内收口剩余页面与验收项 |
| TabBar 信息架构 | 首页、发现、社区、我的四个 Tab 已接入真实页面，不再只是骨架 | 四 Tab 架构已稳定，可继续沿当前结构推进 | 保持四 Tab，不新增与主路径无关的入口 |
| 分包页面 | `pages-article`、`pages-user`、`pages-messages`、`pages-about`、`pages-publish` 均已注册，且大部分页面已有实现 | 路由可达性已不再是主阻断项，但仍有少量占位页 | 优先补齐消息中心、树洞投递、导航与关于等未闭环页面 |
| 首页体验 | 首页已有 Hero、搜索入口、公告轮播、推荐/最新内容与下拉刷新 | 已进入“可用且有展示感”的状态，但仍需继续朝方案目标打磨细节 | 继续优化卡片层级、信息密度和首屏节奏 |
| 发现页体验 | 发现页已接入热门文章与分类数据，并提供搜索入口 | 已脱离静态宫格阶段 | 继续补齐热搜词、搜索建议与更完整的发现路径 |
| 社区体验 | 社区页已有说说/树洞列表和点赞、收藏、评论等轻互动 | 基础社区浏览能力已形成 | 继续补齐发布链路、消息通知和更完整的互动反馈 |
| 我的页体验 | 我的页已有登录入口、资料展示、收藏和关注入口 | 用户资产入口基本成形，但消息中心、资料编辑仍未闭环 | 优先补齐消息中心与可编辑资料流 |
| Liquid Glass UI | 视觉 token、页面壳和卡片体系已部分落地，已明显区别于普通蓝白卡片样式 | 设计语言已进入实现阶段，但还不是最终完成态 | 继续统一细节样式和跨页面一致性 |
| 接口与状态 | 已存在共享请求层、业务域 API、分页 composable 和登录续做能力 | 分层方向基本正确，但仍需继续收口到统一边界 | 新增能力继续遵守业务域分层，不回退到页面直连请求 |

### 7.2 产品调整原则

本次校准不降低产品定位。当前实现已经完成一部分真实业务页面，但还没有达到“阶段全部闭环即可对外总结”的要求。后续应继续以产品方案为目标推进，不把“已有页面”误判成“阶段完成”。需要坚持以下原则：
...
- [ ] 搜索与互动
  - 原因：虽然关键词搜索、点赞、收藏、评论、关注和登录续做已有代码，但搜索联想、热搜/历史与更完整的阶段验收项尚未闭合，暂不标记完成。
- [ ] 用户资产
  - 原因：资料页当前以只读展示为主，消息中心仍是“建设中”，未形成完整用户资产闭环。
- [ ] 社区扩展
  - 原因：社区列表已存在，但树洞投递、网站导航、关于等页面仍有占位或待接入内容。
- [ ] 兼容与性能
  - 原因：缺少真机适配、H5 适配、构建产物与性能专项验证证据。
- [ ] 联调验收
  - 原因：缺少全链路联调、缺陷收口、上线检查与发布验收记录。
...
推文生成口径与此保持一致：任一阶段只要还出现“不通过示例”对应问题，或存在同等级未闭环项，就不得生成该阶段推文。

### 7.5 当前需特别关注的问题

- `sourcelin-ui/sourcelin-ui-uniapp/src/manifest.json` 已出现具体微信 AppID，对外公开前应改为占位或本地配置注入，避免公开分支配置暴露。
- `node_modules`、`dist`、`unpackage` 等生成目录不得提交；当前子项目 `.gitignore` 已覆盖，但提交前仍需做公开边界检查。
- 当前 README 描述 `manifest.json/pages.json` 在项目根目录，实际文件位于 `src/`，后续应同步修正启动说明。
- `ApiResponse`、`PageResult` 类型应集中到共享类型层，并与仓库 `rules/api-contract.md` 保持一致，避免后续页面各自定义响应结构。
...
| 能力 | 建议接口 | 优先级 | 说明 |
|---|---|---|---|
| 举报 | `POST /front/reports` | P1 | 接收目标类型、目标 ID、原因、补充说明和截图；后台审核逻辑可后续迭代 |
