...
- `api-contract-governance`：统一响应体、分页、错误码、前后端消费和 smoke 验证治理。
...
4. 按技能步骤执行，不跳过验证。
5. 若规则、技能、文档发生变化，执行 `node scripts/architecture-guard/rules-system-guard.mjs`。
...
- CHECK：执行 `node scripts/architecture-guard/rules-system-guard.mjs`。
