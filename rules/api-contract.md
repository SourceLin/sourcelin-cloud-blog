...
- CHECK：执行 `node scripts/api-contract-scan.mjs`，阻断项必须为 0。
...
- MUST NOT：前端或脚本使用 `items ?? list`、`items ?? rows`、`items ?? records` 兼容旧字段。
- CHECK：执行 `node scripts/api-contract-scan.mjs`，确认旧分页字段与 fallback 均未出现。
...
- CHECK：审查 Controller 返回类型，并执行 `node scripts/api-contract-scan.mjs`。
...
- CHECK：执行 `node scripts/api-contract-scan.mjs`，确认只存在允许的内部 `R<T>`。
...
- CHECK：执行 `node scripts/api-contract-scan.mjs` 和对应前端类型检查命令。
