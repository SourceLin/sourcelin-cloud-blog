...
- CHECK：执行 `node scripts/architecture-guard/frontend-admin-guard.mjs`。
...
- CHECK：执行 `node scripts/api-contract-scan.mjs` 和 `pnpm run type-check`。
...
- CHECK：后台模块化改动后执行 `pnpm run type-check`、`pnpm run lint`、`node scripts/architecture-guard/frontend-admin-guard.mjs`。
